...
public class BroadSocket {
    ...
    public void handleOpen(Session userSession){
    	System.out.println(">>> broadsocket session add ");
    ...
    public void handleMessage(String message,Session userSession) throws IOException
    {
        ...
        if(username == null){
            ...
            userSession.getBasicRemote().sendText(buildJsonData("Y", message, message + " 님 대화방에 입장하였습니다."));
            return;
        }
        ...
        while(iterator.hasNext()){
        	iterator.next().getBasicRemote().sendText(buildJsonData("N",username,message));            
        }
    }
    /**
     ...
     * @return
     */
    public String buildJsonData(String create, String username, String message){
...
//        return stringwriter.toString();
        ...
        String yymmddss = (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date));

		
    	JSONObject obj = new JSONObject();
		try 
		{
			obj.accumulate("create", create);
			obj.accumulate("username", username);
			obj.accumulate("message", message);
			obj.accumulate("yymmddss", yymmddss);
		} 
